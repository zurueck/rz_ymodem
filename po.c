...
int flash_write(char *ymodemBuf, unsigned int addr, int leng)
{
    
    return 0;
}

int env_set(int index, char* data, int len)
{
    return 0;    
}
...
//extern mdelay(int ms);
#define YM_MODEM 2

static unsigned long YM_load_serial_ymodem(void)
{
	int size;
	int err;
	int res;
	...
	if (!res) {
		while ((res = xyzModem_stream_read(ymodemBuf, 1024, &err)) > 0) {
			...
			store_addr = addr + FILE_BASE_ADDRESS;
			size += res;
			addr += res;
            ...
            if (rc) {
                printf("YMdebug flash write fail\n");
                env_set(INDEX_YMODEM_SUCCEED, 0, 1);
                break;
			}
		}
	} else {
    ...
    env_set(INDEX_FILE_SIZE, (char*)&size, 4);
    env_set(INDEX_YMODEM_SUCCEED, "1", 1);
